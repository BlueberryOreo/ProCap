...
from utils.utils import Timer, DebugLogger
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    debug_logger = DebugLogger(log_path=os.path.join(log_dir, "timer"), file_name=f'gpu_{gpu}_time.log', add_time=False, visible=False)
    
    with Timer(f"main_worker", logger=debug_logger):
        ...
        print("Initializing distributed training on rank", args.rank, "seed", args.seed)
        if is_root_worker():
            for k, v in args._get_kwargs():
                print(f'{k} = {v}')
        ...
        with Timer(f"Loading vae model", logger=debug_logger):
            # get vae model
        ...
        # if dalle_path is given, load weights and resume training
        if dalle_path is not None:
            assert exists(dalle_path), 'DALLE model file does not exist'
            print("** Loading DALL-E model from", dalle_path)
            ckpt = torch.load(str(dalle_path), map_location='cpu')
            model_weights, optim_weights = ckpt['weights'], ckpt.get('optimizer', None)
            start_iter = ckpt.get('iter', start_iter)
        ...
        with Timer(f"Initializing DALL-E model", logger=debug_logger):
                ...
                for state in opt.state.values():
                    for k, v in state.items():
                        if isinstance(v, torch.Tensor):
                            state[k] = v.cuda(args.gpu) if args.gpu is not None else v
        ...
        if args.lr_decay:
            _, scheduler_step = prepare_lr_scheduler(args, opt, start_iter=start_iter)

        # create dataset and dataloader
        args.is_shuffle = True

        with Timer(f"Creating dataset", logger=debug_logger):
            ds = get_dataset(args, tokenizer)
            assert len(ds) > 0, 'dataset is empty: {}'.format(len(ds))
        ...
        with Timer(f"Creating DataLoader", logger=debug_logger):
            dl = DataLoader(
                ds,
        ...
        pbar = tqdm(range(args.iters),
                    initial=start_iter,
                    dynamic_ncols=True,
                    smoothing=0.01, disable=not is_root_worker())

        # training
        for idx in pbar:
            with Timer(f"Step {idx + start_iter}", logger=debug_logger):
                i = idx + start_iter
                which_iter = f"{i:07d}"
                ...
                with Timer(f"Fetching next batch", logger=debug_logger):
                    if args.negvc:
                ...
                with Timer(f"Processing text", logger=debug_logger):
                    if args.fixed_language_model is not None:
                ...
                with Timer(f"Processing visuals", logger=debug_logger):
                    # Train dalle
                ...
                with Timer(f"Backward pass", logger=debug_logger):
                    opt.zero_grad()
                    loss.backward()
                ...
                with Timer(f"Logging", logger=debug_logger):
                    if is_root_worker():
                ...
                with Timer(f"Visualizing", logger=debug_logger):
                    # =================== visualization ======================
                ...
                with Timer(f"Scheduler step", logger=debug_logger):
