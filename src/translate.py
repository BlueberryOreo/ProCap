...
def run_translate(eval_data_loader, translator, opt, is_main_process=True, profiler=None):
    # submission template
    batch_res = {"version": "VERSION 1.0",
                 "results": defaultdict(list),
                 "external_data": {"used": "true", "details": "ay"},
                 "tps": [],  # speed (tokens per second)
                 "spc": [],  # speed (second per caption)
                 }
    for raw_batch in tqdm(eval_data_loader, mininterval=2, desc="  - (Translate)", disable=not is_main_process):
        ...
        if opt.recurrent:
            ...
            if opt.untied or opt.mtrans:
                model_inputs = [b.to(translator.device) for b in raw_batch[1:]]
            else:
                model_inputs = [
            ...
            if opt.dset_name == "clevr":
                dec_seq, no_dec_seq, time_list = translator.translate_batch(
                    model_inputs, use_beam=opt.use_beam, recurrent=False, untied=opt.untied or opt.mtrans, time_list=[time.time()])
                elapsed = time_list[1] - time_list[0] + 1e-6  # avoid division by zero
                tokens = dec_seq.shape[0] * dec_seq.shape[1]
                batch_res["tps"].append(tokens / elapsed)
                batch_res["spc"].append(elapsed / dec_seq.shape[0])

                for example_idx, (name, cur_gen_sen, no_cur_gen_sen) in enumerate(zip(names, dec_seq, no_dec_seq)):
                    cur_data = {
                        "sentence": eval_data_loader.dataset.convert_ids_to_sentence(
                    ...
                    no_cur_data = {
                        "sentence": eval_data_loader.dataset.convert_ids_to_sentence(
                            no_cur_gen_sen.cpu().tolist()).encode("ascii", "ignore"),
                    }
                    batch_res["results"][name + "c"].append(cur_data)
                    batch_res["results"][name + "nc"].append(no_cur_data)

            else:
                dec_seq, time_list = translator.translate_batch(
                    model_inputs, use_beam=opt.use_beam, recurrent=False, untied=opt.untied or opt.mtrans, time_list=[time.time()])
                elapsed = time_list[1] - time_list[0] + 1e-6  # avoid division by zero
                tokens = dec_seq.shape[0] * dec_seq.shape[1]
                batch_res["tps"].append(tokens / elapsed)
                ...
                for example_idx, (name, cur_gen_sen) in enumerate(zip(names, dec_seq)):
                    cur_data = {
                        "sentence": eval_data_loader.dataset.convert_ids_to_sentence(
                            cur_gen_sen.cpu().tolist()).encode("ascii", "ignore"),
                    }
                    batch_res["results"][name].append(cur_data)
...
def run_translate_llm(eval_data_loader, model, opt, is_main_process=True, profiler=None):
    model.eval()
    batch_res = {"version": "VERSION 1.0",
                 "results": defaultdict(list),
                 "external_data": {"used": "true", "details": "ay"},
    ...
    for raw_batch in tqdm(eval_data_loader, mininterval=2, desc="  - (Translate)", disable=not is_main_process):
        if opt.dset_name == "clevr":
            raise NotImplementedError("Translate for LLM on CLEVR-Change dataset is not implemented yet. ")
        else:
            image_id, change_feat, change_ids, change_labels, change_mask = raw_batch
            start_time = time.time()
            dec_tokens, dec_sents = model.generate(change_feat)
            elapsed = time.time() - start_time
            tokens = dec_tokens.shape[0] * dec_tokens.shape[1]
            batch_res["tps"].append(tokens / elapsed)
            batch_res["spc"].append(elapsed / dec_tokens.shape[0])
            for name, sent in zip(image_id, dec_sents):
                batch_res["results"][name].append({
                    "sentence": sent
                })
...
def main():
    ...
    parser.add_argument("--eval_splits", type=str, nargs="+", default=["val", ],
                        choices=["val", "test", "train"], help="evaluate on val/test set, yc2 only has val")
    ...
    if opt.dset_name == "anet":
        ...
        reference_files_map = {"val": [os.path.join(opt.data_dir, "yc2_val_anet_format_para.json")],
                               "train": [os.path.join(opt.data_dir, "yc2_train_anet_format_para.json")]}
    for eval_mode in opt.eval_splits:
        ...
        eval_command = ["python2", "para-evaluate.py", "-s", pred_file, "-o", lang_file,
                        "-v", "-r"] + eval_references
        ...
        eval_rep_cmd = ["python2", "evaluateRepetition.py", "-s", pred_file,
