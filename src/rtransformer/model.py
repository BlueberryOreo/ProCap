...
class VideoFeatureExtractor3D(nn.Module):
    def __init__(self, frame_num, in_resolusion, in_channels, out_dim, dropout=0.1):
        super(VideoFeatureExtractor3D, self).__init__()
        ...
        # self.linear = nn.Linear(128 * (frame_num // 4) * in_resolusion[0] // 8 * in_resolusion[1] // 8, out_dim)
        linear_in_dim = 128 * (frame_num // 4) * in_resolusion[0] // 8 * in_resolusion[1] // 8
        self.feedforward = nn.Sequential(
            nn.Linear(linear_in_dim, (linear_in_dim + out_dim) // 2),
            nn.Dropout(dropout),
            nn.Linear((linear_in_dim + out_dim) // 2, out_dim)
        )
        self.relu = nn.ReLU()
    
    def forward(self, video_clip, output_all_encoded_layers=False):
        features = []
        x = self.relu(self.conv1(video_clip))
        x = self.maxpool1(x)
        features.append(x)
...
class VideoFeatureExtractor2D(nn.Module):
    def __init__(self, config):
        ...
        self.frame_num = config.max_v_len
        self.out_dim = config.hidden_size
        self.resnet101 = models.resnet101(pretrained=True)
        self.resnet101.fc = nn.Linear(self.resnet101.fc.in_features, config.hidden_size)
        ...
        # for param in self.resnet101.fc.parameters():
        #     param.requires_grad = True

        scale = config.hidden_size ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(1, config.hidden_size))
        self.positional_embedding = PositionEncoding(config.hidden_size)
        self.temporal_encoder = BertEncoder(config)
    ...
    def forward(self, batch):
        ...
        std = torch.tensor([0.229, 0.224, 0.224], device=batch.device).view(1, 3, 1, 1)
        features = []
        for video_clip in batch:
            video_clip = (video_clip.permute(1, 0, 2, 3) - mean) / std
            x = self.resnet101(video_clip)
            features.append(x)
        features = torch.stack(features, dim=0)  # (B, T, D)

        features = self.layer_norm(features)

        # add positional embedding
        features = torch.cat([self.class_embedding.type_as(features) + torch.zeros(features.shape[0], 1, features.shape[-1], device=features.device, dtype=features.dtype), 
                              features], dim=1)
        pos_seq = self.positional_embedding(features)
        attention_mask = torch.ones(pos_seq.shape[0], pos_seq.shape[1], pos_seq.shape[1], device=pos_seq.device)
        # print(attention_mask.shape)
        output = self.temporal_encoder(pos_seq, attention_mask, output_all_encoded_layers=False)
...
class BertTextEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.word2idx = json.load(open(config.word2idx_path, 'r'))
        self.vocab_size = len(self.word2idx)
        self.embeddings = nn.Parameter(torch.randn(self.vocab_size, config.hidden_size))
    
    def forward(self, text_input_ids, text_masks):
        text_embedding = F.embedding(text_input_ids, self.embeddings)
        text_masks = torch.ones(text_embedding.shape[0], text_embedding.shape[1], text_embedding.shape[1], device=text_embedding.device)
        output = self.encoder(text_embedding, text_masks, output_all_encoded_layers=False)
...
if __name__ == "__main__":
    # test the model
    frame_num = 9
    video = torch.randn(2, frame_num, 3, 224, 224).cuda()
    video = video.permute(0, 2, 1, 3, 4) # (B, T, C, H, W) -> (B, C, T, H, W)
    model = VideoFeatureExtractor2D(base_config).cuda()
    output = model(video)
