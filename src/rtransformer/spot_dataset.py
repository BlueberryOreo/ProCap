...
class RecursiveCaptionDataset(Dataset):
    ...
    def __init__(self, dset_name, data_dir, video_feature_dir, word2idx_path,
        ...
        change_path = os.path.join(self.video_feature_dir, mode)
    ...
    def __getitem__(self, index):
        ...
        image_id = image_item["img_id"]
        image_sent = image_item["sentences"]
        # if isinstance(image_sent, list):
        #     image_sent = random.sample(image_sent, 1)[0]
...
# TODO: Implement the VideoCaptionDataset class
class VideoCaptionDataset(RecursiveCaptionDataset):
    def __init__(self, dset_name, data_dir, video_feature_dir, word2idx_path, 
        ...
        self.idx2word = {int(v): k for k, v in self.word2idx.items()}
        self.data_dir = data_dir  # containing training data
        self.video_feature_dir = video_feature_dir  # a path contains spot_videos.h5 file
        self.max_seq_len = max_v_len + max_t_len
        self.max_v_len = max_v_len
        self.max_t_len = max_t_len  # sen
        self.tokenizer = tokenizer
        ...
        self.raw_data = load_json(os.path.join(data_dir, "reformat_%s.json" % mode)) # reformat_%s.json

        image_id = {}
        for d in self.raw_data:
            image_id[d["img_id"]] = len(image_id)
        ...
        # >>> Load h5py file
        self.video_path = os.path.join(video_feature_dir, "spot_videos.h5")
        # video_path = os.path.join(video_feature_dir, file_name)
        self.data = h5py.File(self.video_path, "r")
    ...
    def get_filter_indices(self, vid):
        ...
        indices = torch.cat([torch.tensor([0]), torch.sort(indices)[0], torch.tensor([self.num_frames - 1])], dim=0)
        
        return indices
    
    def __getitem__(self, index):
        cap_item = self.raw_data[index]
        image_id = cap_item["img_id"]
        # image_sent = random.choice(cap_item["sentences"]) if self.mode == "train" else cap_item["sentences"][0]
        # _, change_feat = self.data[self.mapper["vid_to_idx"][str(image_id)]]
        ...
        #     change_feat = torch.from_numpy(f[str(image_id)][:]).type(torch.float32) / 255

        if self.filtered:
            filtered_indices = self.get_filter_indices(image_id)
        ...
        change_ids, change_labels, change_mask, sentence = self.load_sentence(cap_item["sentences"])

        if self.return_raw_sentence:
            return image_id, change_feat, change_ids, change_labels, change_mask, sentence
        return image_id, change_feat, change_ids, change_labels, change_mask
        # return image_id, change_feat, image_sent
