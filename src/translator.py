...
class Translator(object):
    ...
    def translate_batch_single_sentence_untied_greedy(
        ...
        # 保持 max_t_len 和 bsz 提前取出
        config = model.config
        max_t_len = config.max_t_len
        bsz = text_input_ids.size(0)

        # 更高效的零初始化（直接创建新张量）
        text_input_ids = torch.zeros_like(text_input_ids, device=device)       # retains shape, device, dtype
        text_masks = torch.zeros_like(text_masks, device=device)

        # next_symbols 初始化为 (bsz,) 的常量 start_idx，放在同一设备上
        next_symbols = torch.full((bsz,), start_idx, dtype=torch.long, device=device)
        for dec_idx in range(max_t_len):
            text_input_ids[:, dec_idx] = next_symbols
            text_masks[:, dec_idx] = 1
            _, pred_scores, _ = model.decode(
    ...
    def translate_batch(self, model_inputs, use_beam=False, recurrent=True, untied=False, xl=False, mtrans=False, time_list=[]):
        ...
        if use_beam:
            ...
            if recurrent:
                ...
                if untied or mtrans:
                    if self.opt.dset_name == "clevr":
                        video_features, text_input_ids, text_input_labels, masks, \
                            no_video_features, no_text_input_ids, no_text_input_labels, no_masks = model_inputs
                        video_masks = masks[:, :self.max_v_len]
                        text_masks = masks[:, self.max_v_len:]
                        no_video_masks = no_masks[:, :self.max_v_len]
                        no_text_masks = no_masks[:, self.max_v_len:]

                        dec = self.translate_batch_single_sentence_untied_greedy(
                            video_features, video_masks, text_input_ids, text_masks, text_input_labels, self.model)
                        if time_list:
                            time_list.append(time.time())
                        no_dec = self.translate_batch_single_sentence_untied_greedy(
                            no_video_features, no_video_masks, no_text_input_ids, no_text_masks, no_text_input_labels,
                            self.model)
                        if time_list:
                            time_list.append(time.time())
                        
                        if time_list:
                            return dec, no_dec, time_list
                        return dec, no_dec
                    else:
                        video_features, text_input_ids, text_input_labels, masks = model_inputs
                        video_masks = masks[:, :self.max_v_len]
                        text_masks = masks[:, self.max_v_len:]
