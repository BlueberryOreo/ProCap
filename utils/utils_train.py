...
def get_dataset(args, tokenizer):
    ...
    if args.dataset == 'video_text':
        from src.mmvid_pytorch.loader import TextVideoDataset
        ds = TextVideoDataset(
        ...
        from src.mmvid_pytorch.loader import TextMP4Dataset
        ds = TextMP4Dataset(
            args.image_text_folder,
            text_len=args.text_seq_len,
            image_size=args.image_size,
        ...
        from src.mmvid_pytorch.loader import TextImageStackDataset
        ...
        from src.mmvid_pytorch.loader_ext import ShapeAttrDataset
        ds = ShapeAttrDataset(
            args.image_text_folder,
            text_len=args.text_seq_len,
            image_size=args.image_size,
        ...
        from src.mmvid_pytorch.loader_ext import VoxDataset
        ds = VoxDataset(
            args.image_text_folder,
            text_len=args.text_seq_len,
            image_size=args.image_size,
        ...
        from src.mmvid_pytorch.loader_ext import iPERDataset
        ds = iPERDataset(
            args.image_text_folder,
            text_len=args.text_seq_len,
            image_size=args.image_size,
    ...
    elif args.dataset == "h5_text":
        from src.mmvid_pytorch.loader import TextH5Dataset
        ds = TextH5Dataset(
            args.image_text_folder,
            text_len=args.text_seq_len,
            image_size=args.image_size,
...
def get_tokenizer(args):
    if args.which_tokenizer == 'simple':
        from src.mmvid_pytorch.tokenizer import SimpleTokenizer
...
def prepare_lr_scheduler(args, optimizer, start_iter=-1):
    if args.lr_scheduler == 'reducelronplateau':
        ...
        scheduler = WarmupDecayLR(
            ...
            last_batch_iteration=start_iter,
        )
        ...
        scheduler = WarmupLR(
            optimizer,
            1e-6 if args.dalle_path is None else args.learning_rate,
            args.learning_rate,
            args.lr_scheduler_warmup,
        )
...
def visualize_train(
    ...
    img_tokens = dalle_module.get_image_tokens(frames, False)
    img_tokens = rearrange(img_tokens, "(b t) n -> b t n", b=frames.shape[0])
    img_tokens[:, 1: -1, ...] = dalle_module.image_token_lut["[MASK]"]
    img_tokens = rearrange(img_tokens, 'b t n -> b (t n)')
    ...
    for j in range(N_SAMPLE):
        ...
        # img_tokens = dalle_module.get_image_tokens(frames[j:j + 1, ...], False)
        # img_tokens = rearrange(img_tokens, "(b t) n -> b t n", b=1)
        # img_tokens[:, 1: -1, ...] = dalle_module.image_token_lut["[MASK]"]
        # img_tokens = rearrange(img_tokens, 'b t n -> b (t n)')
        # img_tokens = img_tokens.repeat(N_PER_SAMPLE, 1)
        tmp_img_tokens = img_tokens[j:j + 1, ...].repeat(N_PER_SAMPLE, 1)
        tmp_origin_frames = frames[j:j + 1, ...].repeat(N_PER_SAMPLE, 1, 1, 1, 1)
        ...
        for mp_steps in args.mask_predict_steps:
            ...
            sample_vc, tmp, _ = generate_images(
                ...
                origin_frames=tmp_origin_frames
            )
            if args.visual:
                samples_img.append(
